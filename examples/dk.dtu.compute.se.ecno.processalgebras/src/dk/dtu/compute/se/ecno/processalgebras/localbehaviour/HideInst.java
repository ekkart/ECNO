...
public class HideInst extends AbstractPetrinetBehaviour<dk.dtu.compute.se.emf.processalgebra.HideInst> {


private static boolean hideCond(
		dk.dtu.compute.se.emf.processalgebra.HideInst self,
		ActionDef action, ProcessInstance pos, ProcessInstance neg) {
	if (self.getActions().contains(action)) {
		if (pos == null || neg == null) {
			return false;
		}

		Iterator<EObject> iterator = self.eAllContents();
		...
		return posFound && negFound;
	} else {
		return true;
	}
}

private static ProcessInstance terminating(ProcessInstance self, Collection<ProcessInstance> terminating) {
    ProcessInstance child = self.getChild().get(0);
    if (terminating.contains(child)) {
        return self;
    }
    return null;
}
	
	public HideInst(ExecutionEngine engine, IElementType type,  EObject element) {
		super(engine, (dk.dtu.compute.se.emf.processalgebra.HideInst) element, new int[]{}, new String[]{"t0"}, type );
	}
	...
	public Object getParamAssignment(PetriNetChoice choice, int event, int param) {
		...
		dk.dtu.compute.se.emf.processalgebra.HideInst self = self();
		...
		case 0: {
			...
			case 0:
				switch (param) {      
				case 0: return terminating(self(),a.terminating);			    
                default: return null;
                }
			default: return null;
			}
	...
	public boolean evaluateCondition(PetriNetChoice choice) {
		...
		case 0: {
			dk.dtu.compute.se.ecno.processalgebras.events.Action a = 
                (dk.dtu.compute.se.ecno.processalgebras.events.Action) choice.getEventValues("action");
			return hideCond(self(),a.action,a.pos,a.neg); 
		}
