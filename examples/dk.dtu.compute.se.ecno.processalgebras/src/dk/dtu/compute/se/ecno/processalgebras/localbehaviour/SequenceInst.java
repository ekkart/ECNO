...
public class SequenceInst extends AbstractPetrinetBehaviour<dk.dtu.compute.se.emf.processalgebra.SequenceInst> {


private static ProcessInstance terminating(dk.dtu.compute.se.emf.processalgebra.SequenceInst self,
        Collection<ProcessInstance> terminating) {
    if (self.getRemainder().isEmpty() && self.getChild().size() == 1) {
        if (terminating.contains(self.getChild().get(0))) {
            return self;
        }
    }
    return null;
	...
	public SequenceInst(ExecutionEngine engine, IElementType type,  EObject element) {
		super(engine, (dk.dtu.compute.se.emf.processalgebra.SequenceInst) element, new int[]{}, new String[]{"t0"}, type );
	}
	...
	public Object getParamAssignment(PetriNetChoice choice, int event, int param) {
		...
		dk.dtu.compute.se.emf.processalgebra.SequenceInst self = self();
	...
	public void executeAction(PetriNetChoice choice) {
		...
		case 0: {
			dk.dtu.compute.se.ecno.processalgebras.events.Action a =
                (dk.dtu.compute.se.ecno.processalgebras.events.Action) choice.getEventValues("action");
{
if (!self().getChild().isEmpty()) {

    ProcessInstance child = self().getChild().get(0);
    if (a.terminating.contains(child)) {
        self().getChild().remove(child);
        if (!self().getRemainder().isEmpty()) {
            self().getChild().add(self().getRemainder().remove(0).createInstance());
            return;
        }
    }
}
