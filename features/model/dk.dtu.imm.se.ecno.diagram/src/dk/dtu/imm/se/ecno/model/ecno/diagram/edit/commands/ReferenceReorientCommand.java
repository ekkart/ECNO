...
public class ReferenceReorientCommand extends EditElementCommand {
	...
	protected boolean canReorientSource() {
		...
		ElementType target = getLink().getTarget();
		return EcnoBaseItemSemanticEditPolicy.getLinkConstraints()
				.canExistReference_4001(getLink(), getNewSource(), target);
	}
	...
	protected boolean canReorientTarget() {
		...
		if (!(getLink().eContainer() instanceof ElementType)) {
			return false;
		}
		ElementType source = (ElementType) getLink().eContainer();
		return EcnoBaseItemSemanticEditPolicy.getLinkConstraints()
				.canExistReference_4001(getLink(), source, getNewTarget());
	}
	...
	protected CommandResult reorientSource() throws ExecutionException {
		getOldSource().getReferences().remove(getLink());
		getNewSource().getReferences().add(getLink());
