...
public class CLASS {
	...
	private String getGenModelURI() {
		return ECNO_M2JGeneratorUtil.findURI(this.context.getEcnogenmodel());
	}
	...
	private Map<Integer,List<Integer>> optionalParamAssignments(int t) {
		dk.dtu.imm.se.ecno.pn.ecnonets.Transition transition = (dk.dtu.imm.se.ecno.pn.ecnonets.Transition) getTransition(t);
		EventBinding binding = transition.getEventbinding();
		Map<Integer,List<Integer>> bindingtable = new HashMap<Integer,List<Integer>>(); 
		if (binding != null) {
			EventUses uses = binding.getStructure();
			if (uses != null) {
				for (int e = 0; e < uses.getEventuse().size(); e++) {
					List<Integer> eventusetable = new ArrayList<Integer>();
					EventUse use = uses.getEventuse().get(e);
					for (int p = 0; p < use.getParameter().size(); p++) {
						NamedParameter param = use.getParameter().get(p);
						if (param.isOptional()) {
							eventusetable.add(p);
						}
					}
					if (!eventusetable.isEmpty()) {
						bindingtable.put(e,eventusetable);
					}
				}
			}
		}
	...
	private Map<Integer,Map<Integer,List<Integer>>> optionalParamAssignments() {
		 Map<Integer,Map<Integer,List<Integer>>> table = new HashMap<Integer,Map<Integer,List<Integer>>>();
		for (int t = 0; t < noTransitions; t++) {
			Map<Integer,List<Integer>> bindingtable = optionalParamAssignments(t);
			if (!bindingtable.isEmpty()) {
				table.put(t,bindingtable);
			}
		}
		return table;
	}
